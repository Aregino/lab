...
# Tabela gerenciada, o spark controla ela 
...
# Tabela não gerenciada, quando eu defino o local path da TABELA delta a ser criada 
df.write.format("delta").saveAsTable("external_products", path="abfss://LAB_UQ@onelake.dfs.fabric.microsoft.com/dp700_spark.Lakehouse/Files/external_products")
...
# MAGIC 
# MAGIC -- TEMPORARY VIEW (ou "visão temporária") no Spark, seja no Microsoft Fabric ou em qualquer outro ambiente, só existe durante a sessão ativa do Spark que a criou.
...
# Esse código grava os dados do dispositivo de streaming no formato Delta em uma pasta chamada iotdevicedata. 
# Como o caminho para o local da pasta na pasta Tabelas, uma tabela será criada automaticamente para ela.
...
# MAGIC 
# MAGIC --Esse código consulta a tabela IotDeviceData, que contém os dados do dispositivo da fonte de streaming.
...
df = spark.sql("SELECT * FROM dp700_spark.iotdevicedata LIMIT 1000")
display(df)
